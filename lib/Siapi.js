...
const { logger, models, getAbsoluteAdminUrl, getAbsoluteServerUrl } = require('siapi-utils');
const { createDatabaseManager } = require('siapi-database');
...
const createSiapiFs = require('./core/fs');
...
class Siapi {
  ...
  logStats() {
    ...
    const isEE = siapi.EE === true && ee.isEE === true;
    ...
    infoTable.push(
      ...
      [chalk.blue('Version'), `${this.config.info.siapi} (node ${process.version})`],
  ...
  logFirstStartupMessage() {
    ...
    const adminUrl = getAbsoluteAdminUrl(siapi.config);
  ...
  logStartupMessage() {
    ...
    const serverUrl = getAbsoluteServerUrl(siapi.config);
  ...
  async load() {
    this.app.use(async (ctx, next) => {
      if (ctx.request.url === '/_health' && ['HEAD', 'GET'].includes(ctx.request.method)) {
        ctx.set('siapi', 'You are so French!');
        ctx.status = 204;
      } else {
        await next();
      }
    ...
    this.models['siapi_webhooks'] = webhookModel(this.config);
  ...
  async runLifecyclesFunctions(lifecycleName) {
    ...
    await Promise.all(
      Object.keys(this.plugins).map(plugin => {
        ...
        return execLifecycle(pluginFunc).catch(err => {
          siapi.log.error(`${lifecycleName} function in plugin "${plugin}" failed`);
          siapi.log.error(err);
          siapi.stop();
        });
      })
    ...
    return execLifecycle(adminFunc).catch(err => {
      siapi.log.error(`${lifecycleName} function in admin failed`);
      siapi.log.error(err);
      siapi.stop();
    });
  }
