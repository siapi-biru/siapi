...
const { yup, formatYupErrors } = siapiUtils;
const { isMediaAttribute, isScalarAttribute, getWritableAttributes } = siapiUtils.contentTypes;
...
const createDzValidator = createOrUpdate => (attr, data, { isDraft }) => {
  ...
  validator = yup.array().of(
    yup.lazy(item => {
      const model = siapi.getModel(prop('__component', item));
      const schema = yup
        .object()
        .shape({
          __component: yup
            .string()
            .required()
            .oneOf(Object.keys(siapi.components)),
        })
        .notNull();
...
const createValidateEntity = createOrUpdate => async (model, data, { isDraft = false } = {}) => {
  try {
    ...
    throw siapi.errors.badRequest('ValidationError', { errors: formatYupErrors(e) });
  }
};
