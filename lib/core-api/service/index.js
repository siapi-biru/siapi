 ...
 * @param {{ model: object, siapi: object }} context
 * @returns {object}
 */
const createService = ({ model, siapi }) => {
  const utils = createUtils({ model });

  if (isSingleType(model)) {
    return createSingleTypeService({ model, siapi, utils });
  }

  return createCollectionTypeService({ model, siapi, utils });
};
...
const getLimitConfigDefaults = () => ({
  defaultLimit: _.toNumber(siapi.config.get('api.rest.defaultLimit', 100)),
  maxLimit: _.toNumber(siapi.config.get('api.rest.maxLimit')) || null,
});
