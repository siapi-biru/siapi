...
module.exports = async plugins => {
  ...
  const version = require(join(dir, 'package.json')).dependencies.siapi;

  const pluginArgs = plugins.map(name => `github:siapi-biru/siapi-plugin-${name}`);
  ...
  try {
    ...
    for (let name of plugins) {
      let pkgPath = findPackagePath(`siapi-plugin-${name}`);
