...
const { version: siapiVersion } = require(path.join(__dirname, '../../../package.json'));
...
module.exports = (dir, initialConfig = {}) => {
  ...
  const rootConfig = {
    ...
    uuid: _.get(pkgJSON, 'siapi.uuid'),
    packageJsonSiapi: _.omit(_.get(pkgJSON, 'siapi', {}), 'uuid'),
    info: {
      ...pkgJSON,
      siapi: siapiVersion,
    },
    installedPlugins: getPrefixedDeps('siapi-plugin', pkgJSON),
    installedMiddlewares: getPrefixedDeps('siapi-middleware', pkgJSON),
    installedHooks: getPrefixedDeps('siapi-hook', pkgJSON),
    installedProviders: getPrefixedDeps('siapi-provider', pkgJSON),
  };
