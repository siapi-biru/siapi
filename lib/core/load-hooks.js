...
const loadPluginsHooks = async (plugins, hooks) => {
  for (let pluginName of plugins) {
    const dir = path.resolve(findPackagePath(`siapi-plugin-${pluginName}`), 'hooks');
...
const loadAdminHooks = async hooks => {
  ...
  const dir = path.resolve(findPackagePath('siapi-admin'), hooksDir);
  await loadHooksInDir(dir, hooks);
...
const loadHookDependencies = async (installedHooks, hooks) => {
  for (let hook of installedHooks) {
    const hookDir = path.dirname(require.resolve(`siapi-hook-${hook}`));
...
const mountHooks = (name, files, hooks) => {
  files.forEach(file => {
    ...
    let dependencies = [];
    try {
      dependencies = _.get(require(`siapi-hook-${name}/package.json`), 'siapi.dependencies', []);
    } catch (err) {
      // Silent
    }
