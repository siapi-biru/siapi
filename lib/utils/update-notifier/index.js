...
const geUpdatetMessage = (newVersion, currentVersion) => {
  ...
  const releaseLink = chalk.bold('https://github.com/siapi/siapi/releases');

  return `
A new version of Siapi is available ${currentVersionLog} → ${newVersionLog}
Check out the new the releases at: ${releaseLink}
`.trim();
};

const createUpdateNotifier = siapi => {
  let config = null;
  ...
  try {
    config = new Configstore(
      pkg.name,
      {},
      { configPath: path.join(siapi.dir, '.siapi-updater.json') }
    );
  } catch {
