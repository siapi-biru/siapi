...
const checkCwdIsSiapiApp = name => {
  let logErrorAndExit = () => {
    console.log(
      `You need to run ${yellow(
        `siapi ${name}`
      )} in a Siapi project. Make sure you are in the right directory`
    );
    process.exit(1);
  };
  ...
  try {
    ...
    if (!_.has(pkgJSON, 'dependencies.siapi')) {
...
const getLocalScript = name => (...args) => {
  checkCwdIsSiapiApp(name);

  const cmdPath = resolveCwd.silent(`siapi/lib/commands/${name}`);
  if (!cmdPath) {
    console.log(
      `Error loading the local ${yellow(
        name
      )} command. Siapi might not be installed in your "node_modules". You may need to run "npm install"`
    );
    process.exit(1);
  }
...
// `$ siapi version` (--version synonym)
program.version(packageJSON.version, '-v, --version', 'Output the version number');
program
  .command('version')
  .description('Output your version of Siapi')
  .action(() => {
...
program
  .command('console')
  .description('Open the Siapi framework console')
...
program
  .command('start')
  .description('Start your Siapi application')
...
program
  ...
  .description('Start your Siapi application in development mode')
...
// `$ siapi generate:template <directory>`
program
  .command('generate:template <directory>')
  .description('Generate template from Siapi project')
...
program
  ...
  .description('Builds the siapi admin app')
  .action(getLocalScript('build'));

// `$ siapi install`
program
  .command('install [plugins...]')
  .description('Install a Siapi plugin')
  .action(getLocalScript('install'));

// `$ siapi uninstall`
program
  .command('uninstall [plugins...]')
  .description('Uninstall a Siapi plugin')
